...
trait JsonResponse {
    ...
    public function remoteSuccessResponse($response)
    {
        return response($response, json_decode($response)->code)->header('Content-Type', 'application/json');
    }

    public function remoteFailResponse($response)
    {
        return response($response, json_decode($response)->code)->header('Content-Type', 'application/json');
    }
}
